...
class Network(nn.Module):
    ...
    def forward(self,a, img, txt, mask):
        ...
        #
         t9=torch.zeros([a,2048],device=0).unsqueeze(dim=-1).unsqueeze(dim=-1)
        #
        # t8=self.fc1(t8).unsqueeze(dim=-1).unsqueeze(dim=-1)
         ...
         txt_f3, txt41, txt42, txt43= self.model_txt(txt)  # txt4: batch x 2048 x 1 x 64
        #
        ...
        # txt_f47 = self.max_pool(txt47)
        # txt_f48 = self.max_pool(txt48)

         t81 = torch.zeros([a, 2048],device=0).unsqueeze(dim=-1).unsqueeze(dim=-1)
        #  t81= self.ran2.unsqueeze(dim=-1).unsqueeze(dim=-1).to(device)
        #  t81=t81.repeat_interleave(a,dim=0)
        #
        # t81= self.fc1(t81).unsqueeze(dim=-1).unsqueeze(dim=-1).to(device)
        ...
        # s1 =torch.cosine_similarity(img_f41,img_f42,dim=1)

        # txt_f4 = self.max_pool(torch.cat([txt_f41, txt_f42, txt_f43, txt_f44, txt_f45, txt_f46], dim=2)).squeeze(dim=-1).squeeze(dim=-1).to(device)
        ...
        # txt_f47 = txt_f47.squeeze(dim=-1).squeeze(dim=-1)
        # txt_f48 = txt_f48.squeeze(dim=-1).squeeze(dim=-1)
