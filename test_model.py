...
def test(data_loader, network, args):
    ...
    with torch.no_grad():
        for images, captions, labels, mask in data_loader:
            ...
            image_embeddings, text_embeddings = network(interval,images, captions, mask)
...
def main(model, args):
    ...
    for i in range(0, 50):
        i = i+1
        model_file = os.path.join(args.model_path, str(115+i))+"checkpoint.pth.tar"
        print(model_file)
        ...
        print('Epoch:{}:ac_top1_t2i: {:.5f}, ac_top5_t2i: {:.5f},ac_top10_t2i: {:.5f},'
              'mAP: {:.5f}'.format(
            start, ac_top1_t2i, ac_top5_t2i, ac_top10_t2i, mAP))
