...
from dogtaitujuanji import dogtaitujuanji
...
class Network(nn.Module):
    def __init__(self,args):
        ...
        self.fts = ft(args)
        self.anl=dogtaitujuanji()

        self.ran1= nn.Parameter(torch.rand(1, 2048))
        self.ran2 = nn.Parameter(torch.rand(1, 2048))
        self.fc1 = nn.Sequential(nn.Linear(8192, 2048), nn.ReLU(), nn.Dropout(0.1))
        self.fc2 = nn.Sequential(nn.Linear(8192, 2048), nn.ReLU(), nn.Dropout(0.1))
    ...
    def forward(self,a, img, txt, mask):
        ...
        img_f51=self.anl(list,numm)
        img_f5=torch.cat([img_f51,img_f41,img_f42, img_f43], dim=1)
        txt_f3, txt41, txt42, txt43= self.fts(txt)  # txt4: batch x 2048 x 1 x 64
        #
        ...
        list = t.tolist()
        nu=len(list)
        txt_f51 = self.anl(list,nu).to(device)
